...
日志采用双通道：控制台按 `runtime.log_level` 输出关键信息，日志文件固定保留 DEBUG 级细节。
...
名称匹配会移除空格、换行和制表符，但仍为精确匹配，不做简称推断或模糊匹配。模板校验通过后，脚本清除模板中的全部旧批注，避免历史批注混入本期结果。
...
6. 新增单位只会触发警告；确认需要纳入后，应人工修改模板和配置，再重新运行。
...
```powershell
python main.py --quarter 2026Q2 --template "D:\path\to\最终汇总表.xlsx"
```
...
从使用者确认的当前季度最终汇总表生成下一季度模板。输出默认保存到
`runtime.output_dir`，不会覆盖源文件：
...
程序会自动生成 `2026年第三季度` 模板，并执行：

- 在修改工作簿前检查源汇总表与配置是否一致：固定行报表检查新增、缺失、
  重复和行位移，以及数据区结束行、合计行变化；报表5检查左右合计行，
  报表7按各子表映射检查；
- 更新指定标题区中的季度、截止日期、环比和同比期间；
...
一致性检查会一次性打印全部差异，并在清空、滚转和保存前停止，程序不会
自动增加映射、插入模板行或修改配置。源表标题与 `--quarter` 不一致、滚转
源公式缺少已保存结果或工作表缺失时，也会停止保存并提示人工核对。
